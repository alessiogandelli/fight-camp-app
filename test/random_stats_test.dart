...
void main() {
  group('generateRandomCombo', () {
    ...
    test('generates combos within min/max length', () {
      for (var i = 0; i < 30; i++) {
        final ids = generateRandomCombo(cfg, [tech('a', TechniqueCategory.boxing), tech('b', TechniqueCategory.kicks), tech('c', TechniqueCategory.knees)]);
        expect(ids.length, inInclusiveRange(3, 5));
      }
    });

    test('requires a punch and a kick when configured', () {
      final punches = [tech('p1', TechniqueCategory.boxing), tech('p2', TechniqueCategory.boxing)];
      final kicks = [tech('k1', TechniqueCategory.kicks)];
      for (var i = 0; i < 20; i++) {
        final ids = generateRandomCombo(cfg.copyWith(categories: [TechniqueCategory.boxing, TechniqueCategory.kicks], includeDefense: false), [...punches, ...kicks]);
    ...
    test('excludes defense unless included', () {
      final pool = [tech('d1', TechniqueCategory.defense), tech('p', TechniqueCategory.boxing)];
      for (var i = 0; i < 10; i++) {
        final ids = generateRandomCombo(RandomConfig.def.copyWith(requireKick: false, requirePunch: true), pool);
        expect(ids.contains('d1'), false);
      }
      final withDef = generateRandomCombo(
          RandomConfig.def.copyWith(requirePunch: false, requireKick: false, includeDefense: true, minTechniques: 4, maxTechniques: 4),
          pool);
  ...
  group('stats', () {
    test('computeLoad rounds minutes times rpe', () {
      expect(computeLoad(180 * 5 ~/ 5 * 60 ~/ (180 * 5) + 0, null), 0); // no rpe -> 0
