...
class _PushupCounterSectionState extends State<PushupCounterSection> {
  ...
  Future<void> _start() async {
    ...
    try {
      ...
      final controller = CameraController(desc, ResolutionPreset.low, enableAudio: false, imageFormatGroup: ImageFormatGroup.yuv420);
  ...
  Widget build(BuildContext context) {
    ...
    final calibrated = _detector.delta >= paramsForSensitivity(_sensitivity).minDelta;
    final elapsedSec = _running || _count > 0 ? ((DateTime.now().millisecondsSinceEpoch - _startedAt) / 1000).round() : 0;

    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        Text(l.pushupsHint, style: const TextStyle(fontSize: 12, color: AppColors.mut)),
        ...
        ClipRRect(
          ...
          child: Container(
            ...
            child: _running && _controller?.value.isInitialized == true
                        ...
                        child: IgnorePointer(
                          child: Container(
                            ...
                            child: Container(
                              ...
                              decoration: BoxDecoration(color: AppColors.bg.withAlpha(200), borderRadius: BorderRadius.circular(99)),
                              child: Row(mainAxisSize: MainAxisSize.min, children: [
                                Icon(Icons.circle, size: 8, color: calibrated ? AppColors.go : AppColors.warn),
                                const SizedBox(width: 6),
                                Text(calibrated ? l.pushupsDetecting : l.pushupsMoveCloser,
                                    style: TextStyle(fontSize: 10.5, fontWeight: FontWeight.w700, color: calibrated ? AppColors.go : AppColors.warn)),
                              ]),
                            ),
                          ),
                        ),
                ...
                : Center(
                    child: _error == null
                        ? Icon(Icons.videocam_off_rounded, size: 34, color: AppColors.mut)
                        : Padding(
          ...
          Expanded(
            child: Field(
              ...
              child: SliderTheme(
                ...
                child: Slider(value: _sensitivity.toDouble(), min: 0, max: 100, divisions: 100, onChanged: (v) => setState(() => _sensitivity = v.round())),
              ),
            ),
          ),
        ]),
        ...
        Row(
          ...
          children: [
            AnimatedSwitcher(
              ...
              child: Text('$_count', key: ValueKey(_count), style: const TextStyle(fontSize: 64, fontWeight: FontWeight.w900, fontFeatures: [FontFeature.tabularFigures()])),
            ),
            const SizedBox(width: 8),
            Text(l.pushupsCount, style: const TextStyle(color: AppColors.mut, fontSize: 13)),
            if (elapsedSec > 0) ...[
              const SizedBox(width: 12),
              Text('· ${fmtClock(elapsedSec)}', style: const TextStyle(color: AppColors.mut, fontSize: 13, fontFeatures: [FontFeature.tabularFigures()])),
            ],
          ],
        ),
        const SizedBox(height: 12),
  ...
  Future<void> _save(AppStore store) async {    final l = AppLocalizations.of(context)!;
    final durationSec = math.max(60, ((DateTime.now().millisecondsSinceEpoch - _startedAt) / 1000).round());
    store.addSession(SessionRecord(
      ...
      strength: [StrengthEntry(exercise: l.pushupsExercise, sets: 1, reps: _count)],
    ));
