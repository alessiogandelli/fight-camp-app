...
class StretchRoutineBuilderPage extends StatefulWidget {
  ...
  @override
  State<StretchRoutineBuilderPage> createState() => _StretchRoutineBuilderPageState();
}
      ...
      child: Center(
        child: ConstrainedBox(
          ...
          child: Column(
            ...
            children: [
              Text((widget.routineId == null ? l.routineNew : l.routineEdit).toUpperCase(),
              ...
              LayoutBuilder(builder: (context, constraints) {
                ...
                final left = _RoutineStepsPanel(nameController: _name, steps: _steps, onReorder: (a, b) => setState(() {
                      final tmp = _steps[a];
                      _steps[a] = _steps[b];
                      _steps[b] = tmp;
                    }), onRemove: (i) => setState(() => _steps.removeAt(i)));
                final right = _StretchPanel(techs: stretchTechs, lang: lang, onPick: (tech) => setState(() => _steps.add(tech.id)));
                return wide
...
class _RoutineStepsPanel extends StatelessWidget {
  ...
  const _RoutineStepsPanel({required this.nameController, required this.steps, required this.onReorder, required this.onRemove});
  ...
  Widget build(BuildContext context) {
    ...
    final lang = store.lang;
    final l = AppLocalizations.of(context)!;
    Technique? techOf(String id) => store.data.techniques.where((t) => t.id == id).firstOrNull;
    ...
    return CardWidget(
      child: Column(
        ...
        children: [
          ...
          Text(l.routineExercisesCount(steps.length), style: const TextStyle(fontSize: 11, color: AppColors.mut)),
          const SizedBox(height: 10),
          if (steps.isEmpty)
            Text(l.routineTapHint, style: const TextStyle(fontSize: 12.5, color: AppColors.mut))
          else
                ...
                child: Row(
                  children: [
                    Container(
                      ...
                      child: img != null ? SvgPicture.asset(img, width: 32, height: 32) : const SizedBox.shrink(),
                    ),
                    const SizedBox(width: 10),
                    Expanded(child: Text((tech?.nameIn(lang) ?? l.sessionUnknown).toUpperCase(),
...
class _StretchPanel extends StatelessWidget {
  ...
  const _StretchPanel({required this.techs, required this.lang, required this.onPick});
                      ...
                      children: [
                        Container(
                          ...
                          child: stretchImageFor(tech.id) != null
                              ? SvgPicture.asset(stretchImageFor(tech.id)!, width: 40, height: 40)
                              : const SizedBox.shrink(),
                        ),
                        const SizedBox(height: 6),
                        Text(tech.shortIn(lang).toUpperCase(),
